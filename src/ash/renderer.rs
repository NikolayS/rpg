...
fn timeline_line(snapshots: &[AshSnapshot], width: usize, no_color: bool) -> Line<'_> {
    let bar = "\u{2588}"; // █
...
fn collect_drill_rows(snapshots: &[AshSnapshot], level: &DrillLevel) -> Vec<DrillRow> {
...
fn drill_row_line(row: &DrillRow, is_selected: bool, max_count: u32, no_color: bool) -> Line<'_> {
    let marker = if is_selected { "\u{25b6} " } else { "  " }; // ▶ or two spaces
    let bar_len = if max_count > 0 {
        usize::try_from((u64::from(row.count) * 20 / u64::from(max_count)).max(1)).unwrap_or(1)
    } else {
        1
    };
...
pub fn draw_frame(frame: &mut Frame, snapshots: &[AshSnapshot], state: &AshState, no_color: bool) {
    ...
    let tl_chunks =
        Layout::vertical([Constraint::Length(1), Constraint::Min(0)]).split(timeline_inner);
