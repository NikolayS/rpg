...
where {name_cond} limit 1"
    ...
    if let Ok(messages) = client.simple_query(&idx_sql).await {
        use tokio_postgres::SimpleQueryMessage;
        // Collect: (idx_name, is_primary, is_unique, amname, idx_oid_str, idx_pred, is_valid, con_type, con_oid, nulls_not_distinct, is_deferrable)
        let mut index_rows: Vec<(String, bool, bool, String, String, String, bool, String, String, bool, bool)> = Vec::new();
        for msg in messages {
            if let SimpleQueryMessage::Row(row) = msg {
        ...
        if !index_rows.is_empty() {
            println!("Indexes:");
            for (idx_name, is_primary, is_unique, amname, idx_oid_str, idx_pred, is_valid, con_type, con_oid, nulls_not_distinct, is_deferrable) in &index_rows {
                ...
                if is_exclude {
                    ...
                    println!("    \"{idx_name}\"{type_label} {amname} {col_expr}{nulls_not_distinct_suffix}{pred_suffix}{deferrable_suffix}{invalid_suffix}");
                }
            }
        }
    }
