...
///
/// Identity columns (`GENERATED ALWAYS AS IDENTITY`) are annotated with
/// `generated always as identity` in the stub so the LLM knows never to
/// include them in `INSERT` column lists.
///
/// # Errors
///
...
pub async fn build_schema_context(client: &tokio_postgres::Client) -> Result<String, String> {
    ...
    for msg in client.simple_query(sql).await.map_err(|e| e.to_string())? {
        if let tokio_postgres::SimpleQueryMessage::Row(row) = msg {
            ...
            let identity_kind = row.get(5).unwrap_or("");
            ...
            let identity_suffix = match identity_kind {
            ...
            let _ = writeln!(
                output,
                "  {column} {type_name}{pk_suffix}{identity_suffix},",
            );
        }
    }
...
mod tests {
    ...
    ///
    /// Tuple: `(schema, table, column, type_name, is_pk, identity_kind)`.
    /// `identity_kind` mirrors `pg_attribute.attidentity`: `"a"` = GENERATED
    /// ALWAYS, `"d"` = GENERATED BY DEFAULT, `""` = not an identity column.
    fn build_from_rows(rows: &[(&str, &str, &str, &str, bool, &str)]) -> String {
        let mut output = String::new();
        let mut current_table = String::new();

        for &(schema, table, column, type_name, is_pk, identity_kind) in rows {
            let qualified = format!("{schema}.{table}");
            if qualified != current_table {
                if !current_table.is_empty() {
                    output.push_str(");\n");
    ...
    #[test]
    fn identity_always_column_annotated() {
        let rows = [
            ("public", "orders", "id", "bigint", true, "a"),
            ("public", "orders", "amount", "integer", false, ""),
        ];
        let out = build_from_rows(&rows);
        assert!(
            out.contains("  id bigint primary key generated always as identity,"),
            "expected 'generated always as identity' in: {out}"
        );
        assert!(out.contains("  amount integer,"));
    }

    #[test]
    fn identity_by_default_column_annotated() {
        let rows = [("public", "t", "id", "bigint", true, "d")];
        let out = build_from_rows(&rows);
        assert!(
            out.contains("  id bigint primary key generated by default as identity,"),
            "expected 'generated by default as identity' in: {out}"
        );
    }

    #[test]
    fn non_identity_column_has_no_identity_annotation() {
        let rows = [("public", "t", "id", "bigint", true, "")];
        let out = build_from_rows(&rows);
